# code from https://gist.github.com/kangtastic/08e7e4036232e63f31bcd93183cbe4ab
import os
...
class AES:
    ...
    def encrypt(self, ptext):
        ...
        return self.padby, self.process_blocks(ptext)
        '''
        if len(ptext) % 16 != 0:
            self.padby, ptext = self.pad_iso_10126(ptext)
        return self.padby, self.process_blocks(ptext)

    def decrypt(self, ctext, padby):
        ...
        decrypted = self.process_blocks(ctext, inverse=inverse)
        if padby != 0:
            decrypted = decrypted[:-padby]
        return decrypted

    def pad_iso_10126(self, ptext):
        if self.mode != "CTR":
            padding_length = 16 - len(ptext) % 16
            padding = bytearray(os.urandom(padding_length - 1)) + bytearray([padding_length])
            return len(padding), bytes(ptext + padding)
        else:
            print("CTR mode padding is not implemented yet")
            raise ValueError()
